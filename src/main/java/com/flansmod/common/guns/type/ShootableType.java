...
public abstract class ShootableType extends InfoType {
    ...
    public boolean destroyOnDeploySubmunition = true;

    /** 召唤炮击 */

    public boolean enableCallArtillery = false;
    public String artilleryType = "";
    public int artilleryStartTime = 20;
    public int artilleryHeight = 300;
    public int artillerySpread = 1;
    public int artilleryNum = 1;
    public int artilleryDelay = 1;
    ...
    /** 是否可以被APS摧毁 */
    public boolean canBeDestructByAPS = false;
    ...
    public boolean detonateOnBouncinessLimit = true;

    public int tickToExplodeAfterImpact = -1;

    /**
     * 弹药箱最大能补给的弹匣数量，默认6
     */
    public int maxSupplyClips = 6;
    ...
    protected void read(String[] split, TypeFile file) {
        ...
        try {
            ...
            else if (split[0].equals("DestroyOnDeploySubmunition"))
                destroyOnDeploySubmunition = Boolean.parseBoolean(split[1].toLowerCase());

                //Artillery
            else if (split[0].equals("EnableCallArtillery"))
                enableCallArtillery = Boolean.parseBoolean(split[1].toLowerCase());
            else if (split[0].equals("ArtilleryType"))
                artilleryType = split[1];
            else if (split[0].equals("ArtilleryStartTime"))
                artilleryStartTime = Integer.parseInt(split[1]);
            else if (split[0].equals("ArtilleryDelay"))
                artilleryDelay = Integer.parseInt(split[1]);
            else if (split[0].equals("ArtilleryNum"))
                artilleryNum = Integer.parseInt(split[1]);
            else if (split[0].equals("ArtilleryHeight"))
                artilleryHeight = Integer.parseInt(split[1]);
            else if (split[0].equals("ArtillerySpread"))
                artillerySpread = Integer.parseInt(split[1]);
            else if(split[0].equals("TickToExplodeAfterImpact") || split[0].equals("TickToExplodeAfterOnGround")){
                tickToExplodeAfterImpact = Integer.parseInt(split[1]);
            }
            ...
            else if (split[0].equals("CanBeDestructByAPS"))
                canBeDestructByAPS = Boolean.parseBoolean(split[1].toLowerCase());
            else if (split[0].equals("DetonateOnBouncinessLimit"))
                detonateOnBouncinessLimit = Boolean.parseBoolean(split[1].toLowerCase());
            else if (split[0].equals("BounceLimit"))
                bounceLimit = Integer.parseInt(split[1]);
            else if (split[0].equals("MaxSupplyClips"))
                maxSupplyClips = Integer.parseInt(split[1]);
            else if (split[0].equals("DistortSound"))
                distortSound = Boolean.parseBoolean(split[1]);
