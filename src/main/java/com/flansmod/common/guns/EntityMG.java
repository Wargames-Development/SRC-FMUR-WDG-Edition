	...
	public boolean attackEntityFrom(DamageSource damagesource, float i)
	{
		if (damagesource.damageType.equals("player"))
		{
			...
			if (player == gunner)
			{
				...
				if (!worldObj.isRemote)
				{
					worldObj.spawnEntityInWorld(((ItemBullet)ammo.getItem()).getEntity(worldObj, (EntityLivingBase) player, type.damage, bulletSpeed, false, ammo.getItemDamage(), type, type.bulletSpread,type.bulletSpread));
				}
				if (soundDelay <= 0)
				{
