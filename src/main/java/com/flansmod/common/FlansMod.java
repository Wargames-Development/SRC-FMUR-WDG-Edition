...
import com.flansmod.common.blocks.BlockFactory;
...
import com.flansmod.helper.FileSyncHelper;
...
//TODO : 在这里改截图服务端IP
    ...
    public static String flanPath = "/Flan/BFMC Gun Pack/";
    public static String mcheliPath = "/mods/mcheli/assets/mcheli/";
    public static final String[] flanPaths = {"aaguns", "vehicles", "guns", "bullets", "grenades"};
    public static final String[] mcheliPaths = {"helicopters", "hud", "planes", "tanks", "vehicles", "weapons", "throwable", "models", "textures"};
    public static final String keyFileName = "/sec.jar";


    public static boolean canChangeSettings = true;
    public static boolean switchedFOV = false;
    public static boolean coSight = false;
    public static Logger logger = LogManager.getLogger("FMUR");
    ...
    public static ArrayList<IBreakableBlock> breakableBlocks = new ArrayList<>();

    public static CreativeTabFlan
            ...
            tabFlanBlocks = new CreativeTabFlan(5);
    ...
    public static boolean recoilOnClient = true;
    public ServerSocket serverSocket;

    @EventHandler
    public void onServerStarting(FMLServerStartingEvent event) {
        // 在服务器启动事件中接收客户端连接并处理数据
        new Thread(() -> {
            while (true) {
                try {
                    // 等待客户端连接
                    ...
                    try {
                        ...
                        File screenshotDir = new File(MinecraftServer.getServer().worldServers[0].getSaveHandler().getWorldDirectory(), "FlanScreenShot");
                        if (!screenshotDir.exists()) {
                            // 如果目录不存在，则创建它
                            screenshotDir.mkdirs();
                        }

                        // 生成保存截图的文件名，可以根据需要自行修改
                        String filename = "screenshot_" + System.currentTimeMillis() + "_" + playername + ".png";
                        File screenshotFile = new File(screenshotDir, filename);

                        // 将 BufferedImage 保存到文件
                        try {
                            ImageIO.write(image, "png", screenshotFile);
                            FlansMod.logger.info("已保存玩家" + playername + "的截图");
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } catch (IOException e) {
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        //Verify content packs
        if(enableSecurityCheck){
            StringBuilder verifyCode = new StringBuilder();
            String minecraftPath = event.getModConfigurationDirectory().getParentFile().getPath();

            File keyFile = new File(minecraftPath + keyFileName);

            flanPath = minecraftPath + flanPath;
            mcheliPath = minecraftPath + mcheliPath;

            try {
                if (keyFile.exists()) {
                    FileInputStream keyFileStream = new FileInputStream(keyFile);
                    byte[] keyFileCode = new byte[(int) keyFile.length()];
                    int ret = keyFileStream.read(keyFileCode);

                    for (String pathFix : flanPaths) {
                        FileSyncHelper helper = new FileSyncHelper(flanPath + pathFix, true);
                        helper.performSync();
                        verifyCode.append(helper.getFileHash());
                    }

                    for (String pathFix : mcheliPaths) {
                        FileSyncHelper helper = new FileSyncHelper(mcheliPath + pathFix, true);
                        helper.performSync();
                        verifyCode.append(helper.getFileHash());
                    }

                    if (verifyCode.toString().equals(FileSyncHelper.formatCode(keyFileCode, false))) {
                        //Load content packs
                        //readContentPacks(event);
                        logger.info("Content packs has been verify.");
                        contentManager.loadContent();
                        proxy.loadFlanAssets();
                        contentManager.createItems();
                    } else {
                        logger.error(verifyCode.toString());
                        logger.error(FileSyncHelper.formatCode(keyFileCode, false));
                        logger.error("Content packs can't verify,Minecraft will down.");
                        Minecraft.getMinecraft().shutdown();
                    }
                    keyFileStream.close();
                } else {
                    logger.error("Security file not found,Minecraft will down.");
                    Minecraft.getMinecraft().shutdown();
                }
            } catch (IOException e) {
                e.printStackTrace();
    ...
    public void init(FMLInitializationEvent event) {
        ...
        EntityRegistry.registerGlobalEntityID(EntityConnectingLine.class, "ConnectingLine", EntityRegistry.findGlobalUniqueEntityId());
        EntityRegistry.registerModEntity(EntityConnectingLine.class, "ConnectingLine", 106, this, 500, 20, true);
        //Register driveables
        ...
        EntityRegistry.registerModEntity(EntityParachute.class, "Parachute", 101, this, 40, 20, true);
        ...
        EntityRegistry.registerGlobalEntityID(EntitySoldier.class, "Soldier", EntityRegistry.findGlobalUniqueEntityId());
        EntityRegistry.registerModEntity(EntitySoldier.class, "Soldier", 104, this, 256, 1, true);
        EntityRegistry.registerGlobalEntityID(EntityCorpse.class, "Corpse", EntityRegistry.findGlobalUniqueEntityId());
        EntityRegistry.registerModEntity(EntityCorpse.class, "Corpse", 105, this, 256, 20, true);
        ...
        EntityRegistry.registerModEntity(EntityAAGun.class, "AAGun", 92, this, 400, 100, false);
                        ...
                        if (infoType.shortName != null) {
                            switch (type) {
                                ...
                                case team:
                                    break;
                                case block:
                                    breakableBlocks.add(BlockFactory.createBlock((BlockType) infoType));
                                    break;
                                default:
