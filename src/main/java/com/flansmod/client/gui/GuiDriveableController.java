...
import com.flansmod.common.network.PacketBFMCKeyInput;
...
public class GuiDriveableController extends GuiScreen {
    ...
    @Override
    public void initGui() {
        if (mc.gameSettings.thirdPersonView == 1 && !FlansModClient.isBulletTrackingActive)
    ...
    public void handleMouseInput() {
        ...
        //右键. Fires shells, drops bombs. Is not a holding thing
    ...
    protected void keyTyped(char c, int i) {
        ...
        if (i == KeyInputHandler.missileAccelerateKey.getKeyCode()) {
            FlansMod.getPacketHandler().sendToServer(new PacketMissileMCLOSAccelerate());
        }
        if(i == KeyInputHandler.f10Key.getKeyCode()){
            FlansMod.getPacketHandler().sendToServer(new PacketBFMCKeyInput(EnumBFMCKeyType.f10, mc.thePlayer.getDisplayName()));
        }
        if(i == KeyInputHandler.tauntKey.getKeyCode()){
            FlansMod.getPacketHandler().sendToServer(new PacketBFMCKeyInput(EnumBFMCKeyType.taunt, mc.thePlayer.getDisplayName()));
        }
    }
    ...
    public void updateScreen() {
        if (mc.gameSettings.thirdPersonView == 1 && !FlansModClient.isBulletTrackingActive)
            mc.renderViewEntity = (plane.getCamera() == null ? mc.thePlayer : plane.getCamera());
        else if(!FlansModClient.isBulletTrackingActive) {
            mc.renderViewEntity = mc.thePlayer;
        }
    }
